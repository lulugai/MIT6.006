...
class PriorityQueue:
    ...
    def pop(self):
        ...
        self.heap[1], self.heap[heap_size] = self.heap[heap_size], self.heap[1]
        popped_key = self.heap.pop(heap_size)
        return popped_key
    def min_heapify(self, i):
        ...
        if l <= heap_size and self.heap[l] < self.heap[i]:
            ...
            mininum = i
        if r <= heap_size and self.heap[r] < self.heap[i]:
            mininum = r
        if mininum != i:
            self.heap[i], self.heap[mininum] = self.heap[mininum], self.heap[i]
            self.min_heapify(mininum)
