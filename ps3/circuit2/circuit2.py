...
class BlitRangeIndex(object):
  ...
  def count(self, first_key, last_key):
    ...
    if high_index < len(self.data) and self.data[high_index] == last_key:
      high_index += 1
...
class BST(object):
  ...
  def delete(self, key):
    ...
    if node is self.root:
      ...
      if pseudo_root.left is not None:
        pseudo_root.left.parent = None
    ...
    while node is not None:
      ...
      if AVL._height(node.left) >= 2 + AVL._height(node.right):
        if AVL._height(node.left.left) < AVL._height(node.left.right):
          self._left_rotate(node.left)#left-right case
        self._right_rotate(node)#left-left case
